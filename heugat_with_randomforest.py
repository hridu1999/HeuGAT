...
def args_parse_cmd():
    ...
    parser.add_argument('-el', '--edge_list', nargs='+', default='Internet-Industry-Partnerships', type=str, help='Edge list (filename WITHOUT extension) of reference graph')  #'CiteSeer', 'Cora', 'Internet-Industry-Partnerships', 'PubMed-Diabetes', 'Terrorists-Relation', 'Zachary-Karate'
...
def embeddings_gen(myCls, train_X, train_y, test_X, test_y, input_enc_dim, nodes, fname):
    ...
    # === Build Graph ===
    edgelist = [(int(u), int(v)) for u, v in np.vstack((train_X[:, :2], test_X[:, :2]))]
    G = nx.DiGraph()
    G.add_edges_from(edgelist)

    # Extract full node list from both train and test
    all_node_ids = np.unique(np.concatenate([
        train_X[:, 0], train_X[:, 1],
        test_X[:, 0], test_X[:, 1]
    ])).astype(int)
    ...
    A = csr_matrix(A)
    X = np.eye(len(node_ids))

    y_dummy = np.zeros((len(node_ids), 1))

    X_in = Input(shape=(X.shape[1],))
    A_in = Input(shape=(None,), sparse=True)

    gat_out = GATConv(output_dim, attn_heads=8, concat_heads=True, activation='elu')([X_in, A_in])
    output = Dense(1, activation='sigmoid')(gat_out)

    model = Model(inputs=[X_in, A_in], outputs=output)
    model.compile(optimizer=Adam(learning_rate=1e-3), loss='binary_crossentropy', metrics=['accuracy'])
    model.fit([X, A], y_dummy, batch_size=X.shape[0], epochs=n_epochs, verbose=2)

    intermediate_model = Model(inputs=[X_in, A_in], outputs=gat_out)
    embeddings = intermediate_model.predict([X, A])

    embeds_df = pd.DataFrame(np.column_stack((node_ids, embeddings)))
    embeds_df.to_csv(fname + '.embeds', sep='\t', header=False, index=False)
...
def inference_predictor(train_X, train_y, test_X, test_y, fname):
    ...
    log_clf = RandomForestClassifier(n_estimators=100, random_state=None)
...
def main_prog_flow(myCls, args, edge_list):
    ...
    log_file = open('eval_log_random.txt', 'a')
    ...
    if os.path.isfile(fname+'.embeds'):
     os.remove(fname+'.embeds')
